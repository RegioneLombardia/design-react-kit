...
function clone$1(val, seen, options = defaultCloneOptions$1) {
  ...
  if (Array.isArray(val)) {
    for (out = Array(k2 = val.length), seen.set(val, out); k2--; ) out[k2] = clone$1(val[k2], seen, options);
    return out;
  }
...
var NO_DIFF_MESSAGE$1 = "Compared values have no visual difference.", SIMILAR_MESSAGE$1 = "Compared values serialize to the same structure.\nPrinting internal object structure without calling `toJSON` instead.", build$1 = {};
Object.defineProperty(build$1, "__esModule", { value: true });
var _default$1 = build$1.default = diffSequence$1, pkg$1 = "diff-sequences", NOT_YET_SET$1 = 0, countCommonItemsF$1 = (aIndex, aEnd, bIndex, bEnd, isCommon) => {
  let nCommon = 0;
  for (; aIndex < aEnd && bIndex < bEnd && isCommon(aIndex, bIndex); ) aIndex += 1, bIndex += 1, nCommon += 1;
  return nCommon;
}, countCommonItemsR$1 = (aStart, aIndex, bStart, bIndex, isCommon) => {
  let nCommon = 0;
  for (; aStart <= aIndex && bStart <= bIndex && isCommon(aIndex, bIndex); ) aIndex -= 1, bIndex -= 1, nCommon += 1;
  return nCommon;
}, extendPathsF$1 = (d2, aEnd, bEnd, bF, isCommon, aIndexesF, iMaxF) => {
  let iF = 0, kF = -d2, aFirst = aIndexesF[iF], aIndexPrev1 = aFirst;
  aIndexesF[iF] += countCommonItemsF$1(aFirst + 1, aEnd, bF + aFirst - kF + 1, bEnd, isCommon);
  ...
  for (iF += 1, kF += 2; iF <= nF; iF += 1, kF += 2) {
    ...
    aIndexPrev1 = aIndexesF[iF], aIndexesF[iF] = aFirst + countCommonItemsF$1(aFirst + 1, aEnd, bF + aFirst - kF + 1, bEnd, isCommon);
  }
  return iMaxF;
}, extendPathsR$1 = (d2, aStart, bStart, bR, isCommon, aIndexesR, iMaxR) => {
  let iR = 0, kR = d2, aFirst = aIndexesR[iR], aIndexPrev1 = aFirst;
  aIndexesR[iR] -= countCommonItemsR$1(aStart, aFirst - 1, bStart, bR + aFirst - kR - 1, isCommon);
  ...
  for (iR += 1, kR -= 2; iR <= nR; iR += 1, kR -= 2) {
    ...
    aIndexPrev1 = aIndexesR[iR], aIndexesR[iR] = aFirst - countCommonItemsR$1(aStart, aFirst - 1, bStart, bR + aFirst - kR - 1, isCommon);
  }
  return iMaxR;
}, extendOverlappablePathsF$1 = (d2, aStart, aEnd, bStart, bEnd, isCommon, aIndexesF, iMaxF, aIndexesR, iMaxR, division) => {
  let bF = bStart - aStart, aLength = aEnd - aStart, baDeltaLength = bEnd - bStart - aLength, kMinOverlapF = -baDeltaLength - (d2 - 1), kMaxOverlapF = -baDeltaLength + (d2 - 1), aIndexPrev1 = NOT_YET_SET$1, nF = d2 < iMaxF ? d2 : iMaxF;
  for (let iF = 0, kF = -d2; iF <= nF; iF += 1, kF += 2) {
    let insert = iF === 0 || iF !== d2 && aIndexPrev1 < aIndexesF[iF], aLastPrev = insert ? aIndexesF[iF] : aIndexPrev1, aFirst = insert ? aLastPrev : aLastPrev + 1, bFirst = bF + aFirst - kF, nCommonF = countCommonItemsF$1(aFirst + 1, aEnd, bFirst + 1, bEnd, isCommon), aLast = aFirst + nCommonF;
    if (aIndexPrev1 = aIndexesF[iF], aIndexesF[iF] = aLast, kMinOverlapF <= kF && kF <= kMaxOverlapF) {
      let iR = (d2 - 1 - (kF + baDeltaLength)) / 2;
      if (iR <= iMaxR && aIndexesR[iR] - 1 <= aLast) {
        let bLastPrev = bF + aLastPrev - (insert ? kF + 1 : kF - 1), nCommonR = countCommonItemsR$1(aStart, aLastPrev, bStart, bLastPrev, isCommon), aIndexPrevFirst = aLastPrev - nCommonR, bIndexPrevFirst = bLastPrev - nCommonR, aEndPreceding = aIndexPrevFirst + 1, bEndPreceding = bIndexPrevFirst + 1;
  ...
  return false;
}, extendOverlappablePathsR$1 = (d2, aStart, aEnd, bStart, bEnd, isCommon, aIndexesF, iMaxF, aIndexesR, iMaxR, division) => {
  let bR = bEnd - aEnd, aLength = aEnd - aStart, baDeltaLength = bEnd - bStart - aLength, kMinOverlapR = baDeltaLength - d2, kMaxOverlapR = baDeltaLength + d2, aIndexPrev1 = NOT_YET_SET$1, nR = d2 < iMaxR ? d2 : iMaxR;
  for (let iR = 0, kR = d2; iR <= nR; iR += 1, kR -= 2) {
    let insert = iR === 0 || iR !== d2 && aIndexesR[iR] < aIndexPrev1, aLastPrev = insert ? aIndexesR[iR] : aIndexPrev1, aFirst = insert ? aLastPrev : aLastPrev - 1, bFirst = bR + aFirst - kR, nCommonR = countCommonItemsR$1(aStart, aFirst - 1, bStart, bFirst - 1, isCommon), aLast = aFirst - nCommonR;
    if (aIndexPrev1 = aIndexesR[iR], aIndexesR[iR] = aLast, kMinOverlapR <= kR && kR <= kMaxOverlapR) {
      ...
      if (iF <= iMaxF && aLast - 1 <= aIndexesF[iF]) {
        ...
        if (division.nChangePreceding = d2, d2 === aLast + bLast - aStart - bStart ? (division.aEndPreceding = aStart, division.bEndPreceding = bStart) : (division.aEndPreceding = aLast, division.bEndPreceding = bLast), division.nCommonPreceding = nCommonR, nCommonR !== 0 && (division.aCommonPreceding = aLast, division.bCommonPreceding = bLast), division.nChangeFollowing = d2 - 1, d2 === 1) division.nCommonFollowing = 0, division.aStartFollowing = aEnd, division.bStartFollowing = bEnd;
          ...
          let bLastPrev = bR + aLastPrev - (insert ? kR - 1 : kR + 1), nCommonF = countCommonItemsF$1(aLastPrev, aEnd, bLastPrev, bEnd, isCommon);
  ...
  return false;
}, divide$1 = (nChange, aStart, aEnd, bStart, bEnd, isCommon, aIndexesF, aIndexesR, division) => {
  ...
  if (aIndexesF[0] = aStart - 1, aIndexesR[0] = aEnd, baDeltaLength % 2 === 0) {
    ...
    for (let d2 = 1; d2 <= dMax; d2 += 1) if (iMaxF = extendPathsF$1(d2, aEnd, bEnd, bF, isCommon, aIndexesF, iMaxF), d2 < dMin) iMaxR = extendPathsR$1(d2, aStart, bStart, bR, isCommon, aIndexesR, iMaxR);
    else if (extendOverlappablePathsR$1(d2, aStart, aEnd, bStart, bEnd, isCommon, aIndexesF, iMaxF, aIndexesR, iMaxR, division)) return;
  } else {
    let dMin = ((nChange || baDeltaLength) + 1) / 2, dMax = (aLength + bLength + 1) / 2, d2 = 1;
    for (iMaxF = extendPathsF$1(d2, aEnd, bEnd, bF, isCommon, aIndexesF, iMaxF), d2 += 1; d2 <= dMax; d2 += 1) if (iMaxR = extendPathsR$1(d2 - 1, aStart, bStart, bR, isCommon, aIndexesR, iMaxR), d2 < dMin) iMaxF = extendPathsF$1(d2, aEnd, bEnd, bF, isCommon, aIndexesF, iMaxF);
    else if (extendOverlappablePathsF$1(d2, aStart, aEnd, bStart, bEnd, isCommon, aIndexesF, iMaxF, aIndexesR, iMaxR, division)) return;
  }
  throw new Error(`${pkg$1}: no overlap aStart=${aStart} aEnd=${aEnd} bStart=${bStart} bEnd=${bEnd}`);
}, findSubsequences$1 = (nChange, aStart, aEnd, bStart, bEnd, transposed, callbacks, aIndexesF, aIndexesR, division) => {
  ...
  aStart < aEndPreceding && bStart < bEndPreceding && findSubsequences$1(nChangePreceding, aStart, aEndPreceding, bStart, bEndPreceding, transposed, callbacks, aIndexesF, aIndexesR, division), nCommonPreceding !== 0 && foundSubsequence(nCommonPreceding, aCommonPreceding, bCommonPreceding), nCommonFollowing !== 0 && foundSubsequence(nCommonFollowing, aCommonFollowing, bCommonFollowing), aStartFollowing < aEnd && bStartFollowing < bEnd && findSubsequences$1(nChangeFollowing, aStartFollowing, aEnd, bStartFollowing, bEnd, transposed, callbacks, aIndexesF, aIndexesR, division);
}, validateLength$1 = (name, arg) => {
  if (typeof arg != "number") throw new TypeError(`${pkg$1}: ${name} typeof ${typeof arg} is not a number`);
  if (!Number.isSafeInteger(arg)) throw new RangeError(`${pkg$1}: ${name} value ${arg} is not a safe integer`);
  if (arg < 0) throw new RangeError(`${pkg$1}: ${name} value ${arg} is a negative integer`);
}, validateCallback$1 = (name, arg) => {
  let type5 = typeof arg;
  if (type5 !== "function") throw new TypeError(`${pkg$1}: ${name} typeof ${type5} is not a function`);
};
function diffSequence$1(aLength, bLength, isCommon, foundSubsequence) {
  validateLength$1("aLength", aLength), validateLength$1("bLength", bLength), validateCallback$1("isCommon", isCommon), validateCallback$1("foundSubsequence", foundSubsequence);
  let nCommonF = countCommonItemsF$1(0, aLength, 0, bLength, isCommon);
  if (nCommonF !== 0 && foundSubsequence(nCommonF, 0, 0), aLength !== nCommonF || bLength !== nCommonF) {
    let aStart = nCommonF, bStart = nCommonF, nCommonR = countCommonItemsR$1(aStart, aLength - 1, bStart, bLength - 1, isCommon), aEnd = aLength - nCommonR, bEnd = bLength - nCommonR, nCommonFR = nCommonF + nCommonR;
    aLength !== nCommonFR && bLength !== nCommonFR && findSubsequences$1(0, aStart, aEnd, bStart, bEnd, false, [{ foundSubsequence, isCommon }], [NOT_YET_SET$1], [NOT_YET_SET$1], { aCommonFollowing: NOT_YET_SET$1, aCommonPreceding: NOT_YET_SET$1, aEndPreceding: NOT_YET_SET$1, aStartFollowing: NOT_YET_SET$1, bCommonFollowing: NOT_YET_SET$1, bCommonPreceding: NOT_YET_SET$1, bEndPreceding: NOT_YET_SET$1, bStartFollowing: NOT_YET_SET$1, nChangeFollowing: NOT_YET_SET$1, nChangePreceding: NOT_YET_SET$1, nCommonFollowing: NOT_YET_SET$1, nCommonPreceding: NOT_YET_SET$1 }), nCommonR !== 0 && foundSubsequence(nCommonR, aEnd, bEnd);
  }
}
...
function diffLinesRaw$1(aLines, bLines, options) {
  ...
  for (_default$1(aLength, bLength, isCommon, (nCommon, aCommon, bCommon) => {
...
` : b;
  ...
  return _default$1(aLength, bLength, isCommon, (nCommon, aCommon, bCommon) => {
